...
import { ColorMode, DESIGN_EXTENSIONS_TABS, RESERVED_FONT_KEYS, UNIFORM_SETTINGS_TABS } from '@/constants';
...
type SettingsProps = {
  location: 'projectTool' | 'settings';
};
const Settings: FC<SettingsProps> = ({ location }) => {
  const tabsToRender = useMemo(
    () => (location === 'settings' ? [...DESIGN_EXTENSIONS_TABS, ...UNIFORM_SETTINGS_TABS] : DESIGN_EXTENSIONS_TABS),
    [location]
  );
  ...
  } = useMeshLocation<typeof location, MeshType.SettingsParams>();
      ...
      switch (tabKey) {
        ...
        case 'webhooks':
          return location !== 'settings' ? null : (
            <Webhook webhooks={webhooks} setWebhooks={handleUpdateData(setWebhooks)} />
          );
        default:
          return null;
      }
          ...
          <div className="mb-3">
            <Tabs selectedId={currentTabKey} onSelectedIdChange={tabKey => setCurrentTabKey(tabKey || 'color')}>
              <TabButtonGroup aria-label="Token system settings page for the project" className="mb-6">
                {tabsToRender.map(({ label: tabName, key: tabKey }) => (
                  <TabButton key={tabKey} id={tabKey}>
                    {tabName}
                  </TabButton>
                ))}
              </TabButtonGroup>
              {tabsToRender.map(({ key: tabKey }) => (
