...
const setAllowedGroups = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const allowedGroup = req.body as Type.KVStorage['allowedGroup'];

    const themeData = (await getDataFromKVStorage(projectId)) || {};

    await setDataToKVStorage(projectKey, { ...themeData, allowedGroup }, environmentUrl);
...
export default setAllowedGroups;
